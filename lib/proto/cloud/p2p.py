...
class MapPixels(betterproto.Message):
    ...
    # * 地图像素数据，目前采用 LZ4 压缩;
    # 地图更新采用SLAM地图+分区地图的方式维护，slam地图实时更新，分区地图仅在保存地图、用户手动调整分区时更新； 实时地图:
    # 1byte表示4像素，即1个像素2bit:          0x00 为未知区域          0x01 为张障碍物          0x02
    # 为可清扫区域          0x03 为地毯 分区地图：      1byte表示1像素的方式          低2bit表示像素
    # 0x00 为未知区域              0x01 为张障碍物              0x02 为可清扫区域
    # 0x03 为地毯          高6bit表示房间分区id              每个像素占 1byte，包含房间id、是否是背景等数据.
    # 房间标识说明                  有效房间标识: 0 - 31                  无效房间标识: 大于等于32
    # 特殊房间标识：                  60：没有房间数据                  61：房间间隙
    # 62：代表障碍物                  63：未知的房间标识
...
class MapInfo(betterproto.Message):
    ...
    docks: List[cloud.Pose] = betterproto.message_field(7)
    msg_type: "MapInfoMapMsgType" = betterproto.enum_field(8)
    pixels: "MapPixels" = betterproto.message_field(9, group="MapMsg")
    obstacles: stream.ObstacleInfo = betterproto.message_field(10, group="MapMsg")
    restricted_zones: stream.RestrictedZone = betterproto.message_field(
        11, group="MapMsg"
    )
    room_params: stream.RoomParams = betterproto.message_field(12, group="MapMsg")
    cruise_data: stream.CruiseData = betterproto.message_field(13, group="MapMsg")
    temporary_data: stream.TemporaryData = betterproto.message_field(14, group="MapMsg")
...
class CompleteMap(betterproto.Message):
    ...
    obstacles: stream.ObstacleInfo = betterproto.message_field(10)
    restricted_zones: stream.RestrictedZone = betterproto.message_field(11)
    room_params: stream.RoomParams = betterproto.message_field(12)
    temporary_data: stream.TemporaryData = betterproto.message_field(13)
...
class CompletePath(betterproto.Message):
    """
    *一个路径点包含 5 个字节:byte 1-2:  x 坐标, byte1高字节，最高位为符号位, byte2为低字节;byte 3-4:  y
    坐标，byte3高字节，最高位为符号位, byte4为低字节;byte 5:    flags信息，bit 0-3 类型，0 - 清扫，1 -
    拖地，2 - 扫+拖，3 - 导航，4 - 回充 (TODO: 根据需求完善)bit 4: 状态，0 - 继续上一轨迹点， 1 -
    新轨迹点，与上一轨迹点不连续；
    """

    path: bytes = betterproto.bytes_field(3)
    path_lz4len: int = betterproto.uint32_field(4)
